...
def getfeats(word, o,tag):
    ...
    features = [
        ...
        (o + "punctuation",isPunct(word)),
        (o + "isalphanum",word.isalnum()),
        (o + "hasApostrophe",isAstr(word))
    ]
...
if __name__ == "__main__":
    ...
    j = 0
    print("Writing to results_basic.txt")
    # format is: word gold pred
    with open("results_basic.txt", "w") as out:
        for sent in test_sents:
            for i in range(len(sent)):
                word = sent[i][0]
                gold = sent[i][-1]
    ...
    print("Now run: python conlleval.py results_basic.txt")
